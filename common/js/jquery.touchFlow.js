...
;(function ($) {
	...
	TouchFlow.prototype = {
		...
		touchstart : function (e) {
			...
			obj.posx = obj.startx - $(this).position().left - 5;
			obj.posy = obj.starty - $(this).position().top;

			if(typeof(obj.opts.stopped) === "function" && obj.speedx !== 80) {
			...
			//var now = $("#touchFlow li.current").outerWidth();
			//var touch_last = $("#touchFlow ul").css("transform","translate3d(" + -now + "px, 0px ,0px)");
			//console.log(touch_last);
			//console.log(now);
		},
		...
		pos_control : function () {
			...
			if(!x_chk || !y_chk) {
				...
				if(Math.abs(thisx - x_limit) > 30) {
					this.set_pos({x:x_limit}, this.duration);
				}

				if(Math.abs(thisy - y_limit) > 30) {
