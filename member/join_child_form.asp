...
action_flag		=  Nvl(RP(Trim(Request("param_r1"))), "")		'// mobile
gubun			=  Nvl(RP(Trim(Request("gubun"))), "")			'// C
...
ElseIf action_flag = "mobile" Then
	'14세미만 정보
	cname		= RP(Request("cname"))
	cbirthyear	= RP(Request("cbirthyear"))
	cbirthmonth	= RP(Request("cbirthmonth"))
	cbirthday	= RP(Request("cbirthday"))
	cgender		= RP(Request("cgender"))
...
Else
	...
	CAll f_AlertURL(sRtnMsg, "/mobile/memeber/join.asp")
End If
'=================================================================
%>
<!DOCTYPE html>
...
<!--#include virtual="/mobile/common/inc/header.asp" -->
    ...
    <div class="contents">
        <div class="detail-login">
			...
			<input type="hidden" name="gender" value="<%=cgender%>">
			<input type="hidden" name="mobile1" value="">
			<input type="hidden" name="mobile2" value="">
			<input type="hidden" name="mobile3" value="">
			<input type="hidden" name="pname" value="<%=sName%>">
			<input type="hidden" name="pgender" value="<%=gender%>">

                <label for="name" class="nameLa">이름</label>
                <input type="text" id="cname" class="name" name="cname" placeholder="이름" />
                <label for="sex" class="nameLa">성별</label>
                <input type="text" id="csex" class="csex" name="csex" placeholder="남" maxlength="2" />

                <label for="birth_date_01">생년월일</label>
                <div class="select_box birth_date_box">
                    <div class="select">
						<select name="birthyear" id="birthyear" title="년도 선택" onchange="javascript:setBirthDay('frm');" class="birth_date_01">
							<option value=''>년도</option>
							<%
								toyears = year(date)

								For j = toyears To (toyears - 14) Step -1
							%>
                ...
                <input type="text" id="parent_name" class="parent_name" name="parent_name" placeholder="이름" value="<%=sName%>" />
                ...
                <input type="text" id="parent_sex" class="parent_sex" name="parent_sex" placeholder="남" value="<%=strgender%>" />
                <label for="parent_birth_date_01">보호자 생년월일</label>
                <div class="select_box birth_date_box">
                    <div class="select">
                        <select name="pbirthyear" id="pbirthyear" class="birth_date_01">
							<option value=''>년도</option>
							<%
                    ...
                    <div class="select">
                        <select name="pbirthmonth" id="pbirthmonth" class="birth_date_02">
                            <option value="">월</option>
							<%
								For j = 1 To 12
									If Len(j) < 2 Then
                    ...
                    <div class="select">
                        <select name="pbirthday" id="pbirthday" class="birth_date_03">
					        <option value="">일</option>
							<%
								For k = 1 To 31
									If Len(k) < 2 Then
									...
									End If
							%>
							<option value="<%=k%>"><%=CInt(k)%>일</option>
							<%Next%>
                        </select>
                    </div>
                </div>
...
-->
                ...
                <input type="password" id="password1" class="pw" name="password1" maxlength="14" placeholder="6~14자의 영문,영문+숫자만 사용합니다" />
                <label for="pw_chk">비밀번호 확인</label>
                <input type="password" id="password2" class="pw_chk" name="password2" maxlength="14" placeholder="비밀번호를 한번 더 입력해주십시오" />

                <div class="address_wrap">
                    <label for="add_01">주소</label>
                    <span class="add_01_box">

                        <input type="text" name="zipcode1" id="zipcode1" readonly="readonly" class="add_03" placeholder="우편번호1" style="bottom: 0; float: left; width: 25%; margin: 0;" /><span style="bottom:0;float:left;margin: 0px 5px 0px 5px;">-</span>
                        <input type="text" name="zipcode2" id="zipcode2" readonly="readonly" class="add_03" placeholder="우편번호2" style="bottom: 0; float: left; width: 25%; margin: 0;" />
                        <a id="btnPopDaumPostcode" name="btnPopDaumPostcode" href="#" class="add_search_btn">우편번호 찾기</a>
                    </span>
                    <input type="text" id="address" class="add_02" name="address" readonly="readonly" />
                    <input type="text" id="address_detail" class="add_03" name="address_detail" />&nbsp; (* Daum Kakao Corp. 제공)
                </div>
                ...
                <div class="select_box tel_box">
					...
					<input type="text" name="phone2" id="phone2" class="tel_02" maxlength="4" onKeyUp="this.value=this.value.replace(/[^0-9]/g,'');" onkeypress="txtOnlyNum(event)" style="float:left;width:30%;margin:0;" />
					<span style="bottom:0;float:left;margin:0px 5px 0px 5px;">-</span>
					<input type="text" name="phone3" id="phone3" class="tel_02" maxlength="4" onKeyUp="this.value=this.value.replace(/[^0-9]/g,'')" onkeypress="txtOnlyNum(event)" style="float:left;width:30%;margin:0;" />
                </div>
                ...
                <div class="select_box phone_box">
                    ...
                    <input type="text" id="pmobile2" class="phone_02" name="pmobile2" placeholder="휴대폰 번호" />
                </div>
                <div class="check_box_wrap">
				<!--
                ...
                <div class="select_box email_box">
                    ...
                    <input type="text" id="email1" class="email_01" name="email1" placeholder="이메일 아이디" />
                    <span class="text_at">@</span>
                    <input type="text" id="email2" class="email_02" name="email2" placeholder="이메일 주소" />
                ...
                <span class="cert_btn_box">
                    <a href="javascript:FormInsertCheckM();" class="btn join_ok_btn ok_btn">확인</a>
...
<script>
	...
	$(document).ready(function(){
		...
		$("#pbirthyear").val('<%=birthyear%>');
		$("#pbirthmonth").val('<%=birthmonth%>');

		$("#pmobile1").val('<%=mobile1%>');
		$("#pmobile2").val('<%=mobile2%><%=mobile3%>');
	});
