<article class="popup_wrap id_chk_pop" style="visibility:hidden;">
    <div class="dim-bg"></div>
    <div class="pop">

    <!-- 아이디 중복확인 -->
        <div class="content">
            <h2>아이디 중복확인</h2>
            <label for="address">아이디</label>
            <input type="text" id="address" class="address" name="address" placeholder="6~14자의 영문,영문+숫자 사용" />

            <p class="explan_text center">
                사용하고자 하는 아이디를 입력 후<br/>
                중복확인 버튼을 클릭하세요.
            </p>
            <span class="cert_btn_box">
                <a href="#none" class="btn ok_btn">중복확인</a>
                <a href="#none" class="btn cancel_btn">취소</a>
            </span>
            <a href="#none" class="popup_close-btn">팝업 닫기버튼</a>
        </div>

    <!-- 아이디 사용가능 -->
        <!--<div class="content">-->
            <!--<h2>아이디 중복확인</h2>-->
            <!--<label for="address">아이디</label>-->
            <!--<input type="text" id="address" class="address" name="address" value="abcd1234" readonly="readonly" />-->

            <!--<div class="explan_text02">-->
                <!--<em>사용가능</em>-->
                <!--<span class="text">사용할 수 있는 아이디입니다</span>-->
            <!--</div>-->
            <!--<span class="cert_btn_box">-->
                <!--<a href="#none" class="btn ok_btn">확인</a>-->
                <!--<a href="#none" class="btn cancel_btn">취소</a>-->
            <!--</span>-->
            <!--<a href="#none" class="popup_close-btn">팝업 닫기버튼</a>-->
        <!--</div>-->

    <!-- 아이디 사용불가 -->
        <!--<div class="content">-->
            <!--<h2>아이디 중복확인</h2>-->
            <!--<label for="address">아이디</label>-->
            <!--<input type="text" id="address" class="address" name="address" value="abcd1234" readonly="readonly" />-->

            <!--<div class="explan_text02">-->
                <!--<em>사용불가</em>-->
                <!--<span class="text">이미 사용중인 아이디입니다</span>-->
            <!--</div>-->
            <!--<a href="#none" class="cancel_btn">취소</a>-->
            <!--<a href="#none" class="popup_close-btn">팝업 닫기버튼</a>-->
        <!--</div>-->

    </div>
</article>