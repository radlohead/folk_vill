...
<%
	If session("id") = "" Then
		...
		Response.write " parent.location.href='/mobile/member/login.asp?RtnURL=/mobile/mypage/member_pwd_edit.asp';"
	...
	Dim password : password			= RP(Request("password")) '현재 비밀번호
	Dim password1 : password1		= RP(Request("password1")) '새로운 비밀번호
...
'	Response.write password1 & "<br>"
'	Response.End

	If id = "" Or password = "" Or password1 = "" Then 
		Call f_AlertParentUrl("비정상 접속 입니다.\n\n문제가 있으시면 관리자에게 문의해주세요.", "/mobile/mypage/member_pwd_edit.asp")
	End If 
...
'	response.write "md4-sha"&XTEncrypt.MD5(XTEncrypt.SHA256(password)) & "<br>"
	...
	password1 = XTEncrypt.SHA256(password1)
...
'	Response.End
	...
	If Ret = "1" Then '비밀번호가 일치할 경우

		Call OpenDbConnection() '데이터베이스 오픈
		Conn.BeginTrans() '트랜잭션 시작
		
		'########################################################
		'# TBL_MEMBER PASSWORD Update START
		'########################################################
		...
		With objCmd								
			...
			.ActiveConnection = Conn
			.CommandText = "SP_HUGO7_TBL_MEMBER_PASSWORD_UPDATE"
			.CommandType = adCmdStoredProc
			'기본정보
			.Parameters.Append .CreateParameter("@UID",adVarChar,adParamInput,20, id)
			.Parameters.Append .CreateParameter("@PWD",adVarChar,adParamInput,100, password)
			.Parameters.Append .CreateParameter("@PWD1",adVarChar,adParamInput,100, password1)
		...
		'########################################################
		'# TBL_MEMBER PASSWORD Update END
		'########################################################
		...
		Else
			...
			Response.Write " parent.location.href='http://www.koreanfolk.co.kr/mobile/mypage/member_pwd_finish.asp';"
			Response.Write "</script>"	
			Response.End
		End If	

	ElseIf Ret = "0" Then			' 비밀번호가 일치하지 않을 경우
		Call CloseDbConnection()	' 데이터베이스 클로즈
		Response.Write "<script language='javascript'>"
