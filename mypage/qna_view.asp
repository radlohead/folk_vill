...
<%
	...
	With objcmd
		...
		.CommandText = "SP_HUGO7_TBL_INQUIRY_SELECT_M"
	...
	With objcmd
		.ActiveConnection = Conn
		.CommandText = "SP_HUGO7_TBL_INQUIRY_PREV_SELECT_M"
		.CommandType = adCmdStoredProc
		.Parameters.Append .CreateParameter("@SEQ", adInteger, adParamInput, 0, seq)
		.Parameters.Append .CreateParameter("@id", adVarchar, adParamInput, 20, ss_id)
	...
	With objcmd
		.ActiveConnection = Conn
		.CommandText = "SP_HUGO7_TBL_INQUIRY_NEXT_SELECT_M"
		.CommandType = adCmdStoredProc
		.Parameters.Append .CreateParameter("@SEQ", adInteger, adParamInput, 0, seq)
		.Parameters.Append .CreateParameter("@id", adVarchar, adParamInput, 20, ss_id)
...
<head>
	...
	function Write() {
		location.href = "/mobile/customer/qna_form.asp";
	}
	//-->
	</script>
</head>
        ...
        <div class="detail-mypage">
            ...
            <div class="content qna-view">
                <article>
                 ...
                 <span class="btns">
					<%If prev_seq <> "" Then%>
                    <a href="/mobile/mypage/qna_view.asp?seq=<%=prev_seq%>&pg=<%=pg%>" class="btn prev-btn">이전 글 보기</a>
					<%End If%>
					<%If next_seq <> "" Then%>
                    <a href="/mobile/mypage/qna_view.asp?seq=<%=next_seq%>&pg=<%=pg%>" class="btn next-btn">다음 글 보기</a>
					<%End If%>
                     <span class="rg_btn">
                         <a href="#none" onclick="Del('<%=seq%>','<%=pg%>','<%=ss_id%>');" class="btn del-btn">삭제</a>
						<a href="/mobile/mypage/qna_list.asp?pg=<%=pg%>" class="btn qna-list-btn list-btn">목록</a>
                     </span>
                 </span>
            </div>
        </div>
