...
<%	
	If session("id") = "" Then
		...
		Response.write " parent.location.href='/mobile/member/login.asp?RtnURL=/mobile/customer/qna_form.asp';"
	...
	Dim ip : ip				= RP(Request.ServerVariables("REMOTE_HOST"))	
...
'	Response.End
	...
	'#######################################################
	'# TBL_INQUIRY TABLE Insert START
	'#######################################################

	Set objCmd = Server.CreateObject("ADODB.Command")
	With objCmd
		.ActiveConnection = Conn
		.CommandText = "SP_HUGO7_TBL_INQUIRY_INSERT"
	...
	Else
		...
		'#######################################################
		'# 1:1 상담문의내용 이메일 전송 종료
		'#######################################################

		Response.Write "<script language='javascript'>"
		Response.Write " alert('1:1 상담문의 내용이 등록 되었습니다.\n\n답변은 마이페이지에서 확인하실 수 있습니다.');"
	...
	Sub SendMail(mTo, mToName, mFrom, mFromName, mTitle, mContents, mAnswer, mGubun)
		...
		mSubject = mFromName&"님의 1:1 상담문의 내용입니다."
		
		h_msg = read_file_temp("E:\www\edm\edm_qna.html")
		...
		iMsg.To			= mToName&"<"&mTo&">;"&mToName2&"<"&mTo2&">" 'ToDo: Enter a valid email address.
